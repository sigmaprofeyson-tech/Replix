...
// ---------- MIGRASYON: video_mute kolonu ekle (JSON yuklemeden ONCE olmali!) ----------
try {
...
try {
  ...
  for (const s of scenesData) {
    ...
    } else if (!existing.video_mute && videoMute) {
      // eski kayit var ama sessiz video uretilmemis -> guncelle
      db.prepare('UPDATE scenes SET video_mute=? WHERE id=?').run(videoMute, existing.id);
      console.log('[Sistem] Sahne guncellendi (video_mute eklendi):', s.name);
    }
  }
} catch (err) {
...
function publicState(r) {
  ...
  return {
    ...
    scene: scene ? { 
        ...
        // Burada text garantisi veriyoruz. Frontend text bulamayip undefined basmasin diye.
...
io.on('connection', (sock) => {
  ...
  // karakter dagitimi - SADECE sunucuda, rastgele (tek kisi ise hazirlik sarti aranmaz)
